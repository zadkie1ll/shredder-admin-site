...
- Wata hosted page открывается верхним окном через обычный redirect, без iframe.
  Это важно для СБП: сайты банков часто запрещают встраивание во фрейм, поэтому
  выбор банка внутри iframe ломается. После оплаты Wata возвращает пользователя на
  нашу `payment_status`.
