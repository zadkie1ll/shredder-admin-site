...
- **setup** — подключение по платформам (см. ниже)
- **profile** — реферальная программа
- **support** — чат поддержки

В десктопном сайдбаре внизу есть отдельная кнопка «Выйти».

**Флоу настройки (`setup`).** По умолчанию (`USE_NEW_SETUP_FLOW=false`) показывается
...
- **Герой** экрана — карточка инструкции (`#full-instructions-container`, рендерит
  `setApp`): шаг установки приложения → крупная во всю ширину кнопка-CTA
...
ведущая на этот экран настройки (`showTab('setup')`).
