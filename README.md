...
- **Герой** экрана — карточка инструкции (`#full-instructions-container`, рендерит
  ...
  CTA рендерится **прямым потомком** `.setup-compact-card` (вне колонки шага), поэтому
  занимает всю ширину карточки и выровнен с блоками ниже; шрифт обычного регистра
  (`font-weight:700`, без `uppercase`), чтобы не выглядеть громоздким. Предупреждения шага
...
На вкладке home есть заметная кнопка **«Открыть инструкцию»** (под карточкой статуса),
ведущая на экран установки (`showTab('setup')`).
