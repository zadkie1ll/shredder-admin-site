...
- АГЕНТУ (Claude) КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО лезть в БД напрямую — в ЛЮБУЮ:
  production, staging и локальную дев-БД. Никаких ручных ALTER TABLE /
  CREATE INDEX / DROP / INSERT / UPDATE / DELETE мимо alembic-миграций —
  ни для «быстрой проверки», ни для тестовых данных. Локальная дев-БД —
  эталон для `alembic revision --autogenerate`: ручная правка её схемы
  прячет дифф, и autogenerate генерирует ПУСТУЮ миграцию (инцидент
  2026-09-01: пустая миграция уехала на прод, вкладка админки лежала).
  Миграции генерирует и применяет ТОЛЬКО пользователь своим скриптом.
  Агенту разрешено только читать (SELECT) для диагностики. Нужна новая
  схема или тестовые данные для проверки — попроси пользователя.
